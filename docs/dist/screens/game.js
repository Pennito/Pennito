...
import { MultiplayerSync } from '../network/multiplayer.js';
...
export class GameScreen {
    constructor(canvas, username, worldName, isNew, onExit) {
        ...
        this.multiplayer = null; // Multiplayer sync
        this.otherPlayers = []; // Other players in the world
        this.lastPositionBroadcast = 0; // Throttle position updates
        this.canvas = canvas;
    ...
    async initializeMultiplayer() {
        ...
        this.multiplayer.setChatCallback((username, text) => {
        ...
        this.multiplayer.setWorldUpdateCallback((worldData) => {
    ...
    async listenForGlobalUpdates() {
        const supabase = await getSupabaseClient();
        if (!supabase)
            return;
        try {
            const channel = supabase.channel('global-updates');
            channel.on('broadcast', { event: 'global_message' }, (payload) => {
    ...
    async addChatMessage(username, text) {
    ...
    async handleDevPay(targetUsername, amount) {
