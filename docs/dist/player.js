...
import { PLAYER_WIDTH, PLAYER_HEIGHT, GRAVITY, JUMP_STRENGTH, MOVE_SPEED, MAX_FALL_SPEED, TILE_SIZE, INVENTORY_SLOTS } from './utils/constants.js';
export class Player {
    constructor(x, y, username = 'Player') {
        this.gems = 0; // Player's gem currency
        this.redeemedCodes = []; // Track redeemed codes
        this.equippedShoes = null; // Currently equipped shoes
        this.equippedHat = null; // Currently equipped hat
        this.equippedPants = null; // Currently equipped pants
        this.equippedShirt = null; // Currently equipped shirt
        this.equippedWings = null; // Currently equipped wings (enables double jump)
        this.maxInventorySlots = INVENTORY_SLOTS; // Expandable inventory (starts at 8, max 50)
        this.animationFrame = 0;
        this.lastMoveTime = 0;
        this.isMoving = false;
        this.jumpCount = 0; // Track jumps for double jump (0 = no jump, 1 = first jump, 2 = double jump used)
        this.doubleJumpCooldown = 0; // Cooldown timer after second jump (in ms)
        this.lastCooldownUpdate = 0; // Track time for cooldown updates
        this.DOUBLE_JUMP_COOLDOWN_MS = 500; // 0.5 seconds cooldown
        this.prevJumpHeld = false; // whether jump key was held last frame
        this.lastJumpTime = 0; // timestamp of last successful jump
        this.x = x;
        this.y = y;
        this.velocityX = 0;
        this.velocityY = 0;
    ...
    update(world, moveLeft, moveRight, jumpHeld) {
    ...
    addToInventory(tileType, count = 1) {
    ...
    render(ctx, cameraX, cameraY, worldOwner = null) {
        const screenX = this.x - cameraX;
        const screenY = this.y - cameraY;
        // Player shadow (ellipse)
        ctx.fillStyle = 'rgba(0, 0, 0, 0.3)';
        ctx.beginPath();
        ctx.ellipse(screenX + this.width / 2, screenY + this.height + 1, this.width / 2 + 2, 3, 0, 0, Math.PI * 2);
        ctx.fill();
        // Animation timing
