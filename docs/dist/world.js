...
export class World {
    constructor(width = WORLD_WIDTH, height = WORLD_HEIGHT, seed) {
        this.owner = null; // World owner username (if gold lock is placed)
        this.spawnX = 0; // Spawn door X position (in tiles)
        this.spawnY = 0; // Spawn door Y position (in tiles)
        this.signs = new Map(); // Sign text data (key: "x,y", value: text)
        this.width = width;
        this.height = height;
    ...
    render(ctx, cameraX, cameraY, canvasWidth, canvasHeight, playerX, playerY, playerW, playerH) {
                ...
                if (tile && tile.type !== TileType.AIR) {
                    ...
                    // Special rendering for SIGN
                        ...
                        if (tile.signText && tile.signText.length > 0 &&
                            ...
                            const isPlayerNear = !(tileRight < playerX ||
    ...
    renderTileWithTexture(ctx, type, x, y) {
