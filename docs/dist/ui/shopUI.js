...
export class ShopUI {
    ...
    render(ctx, mouseX, mouseY, gems, redeemCodeInput, maxInventorySlots = 8, inventoryExpansionCost = 50) {
    ...
    renderItemIcon(ctx, tileType, x, y, size) {
        ctx.save();
        switch (tileType) {
    ...
    checkTabClick(mouseX, mouseY) {
        ...
        if (mouseY < tabY || mouseY > tabY + tabHeight)
            return null;
        const tabs = ['clothes', 'blocks', 'essentials'];
        for (let i = 0; i < tabs.length; i++) {
            const tabX = modalX + 20 + (i * tabWidth);
    ...
    checkBuyClick(mouseX, mouseY, maxInventorySlots = 8, inventoryExpansionCost = 50) {
        const modalX = CANVAS_WIDTH / 2 - 400;
        const modalY = 50;
        const modalW = 800;
        const tabY = modalY + 95;
    ...
    checkRedeemClick(mouseX, mouseY) {
        const modalX = CANVAS_WIDTH / 2 - 400;
        const modalY = 50;
        const modalW = 800;
        const modalH = CANVAS_HEIGHT - 100;
