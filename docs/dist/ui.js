...
import { TILE_COLORS, CANVAS_WIDTH, CANVAS_HEIGHT, INVENTORY_SLOTS } from './utils/constants.js';
export class UI {
    constructor(canvas) {
        this.hoveredSlot = -1;
        ...
        this.tooltipX = 0;
        this.tooltipY = 0;
        this.inventoryExpanded = false; // Whether inventory is expanded (pulled up)
        this.inventoryExpansionTarget = 0; // Target expansion height (0 = collapsed, 1 = expanded)
        this.inventoryExpansionCurrent = 0; // Current expansion height (for smooth animation)
        this.canvas = canvas;
    ...
    renderItemIcon(tileType, x, y, size) {
        this.ctx.save();
    ...
    renderInventory(inventory, selectedSlot, maxSlots = INVENTORY_SLOTS, playerGems = 0, expansionCost = 0) {
        // Update animation
        ...
        for (let row = 0; row < visibleRows; row++) {
            for (let col = 0; col < slotsPerRow; col++) {
                ...
                if (slotIndex >= maxSlots)
                    break; // Don't render beyond max slots
                ...
                if (item && item.count > 0) {
                    ...
                    if (item.count > 1) {
                        ...
                        this.ctx.fillText(item.count.toString(), x + slotWidth - 4, y + slotHeight - 4);
                    }
                }
                // Slot number
                this.ctx.fillStyle = '#fff';
                ...
                this.ctx.textAlign = 'center';
                this.ctx.fillText((slotIndex + 1).toString(), x + slotWidth / 2, y + slotHeight + 16);
            }
        }
